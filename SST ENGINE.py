...
def portfolio_impact_map(portfolio, shock, factor_map):
    ...
    for asset in portfolio:
        ...
        factor = factor_map.get(ticker, "Equities")
...
import os
import json
sys.path.append("/home/saksham/Downloads/mars")
from api import DEFAULT_PORTFOLIO
import yfinance as yf

# Compute real-time weights based on live prices
print("Fetching real-time prices for portfolio...")
...
if os.path.exists("current_portfolio.json"):
    try:
        ...
        print(f"Could not load custom portfolio: {e}")
...
for item in portfolio_data:
    ...
    val = price * qty
    total_value += val
    portfolio_items.append({"ticker": ticker, "value": val})

portfolio = []
for p in portfolio_items:
    weight = p["value"] / total_value if total_value > 0 else 0
    portfolio.append({"ticker": p["ticker"], "weight": round(weight, 4)})
...
with open("/home/saksham/Downloads/mars/sst_input.json", "r") as f:
...
output_path = "/home/saksham/Downloads/mars/sst_output.json"
with open(output_path, "w") as f:
    json.dump(result, f, indent=2)

print(f"✅ SST Engine output written to {output_path}")